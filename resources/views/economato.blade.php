            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm" id="tabla-productos">
                    ...
                    <tbody>
                            ...
                            <td class="px-6 py-4">
                                <div class="flex gap-3">
                                    <button onclick="abrirModalEditar({{ $producto->id }}, '{{ addslashes($producto->nombre) }}', '{{ addslashes($producto->tipo) }}', {{ $producto->stock }})"
                                        class="text-yellow-500 hover:text-yellow-400 text-xs uppercase tracking-wider">Editar</button>
                                    <form method="POST" action="{{ route('productos.destroy', $producto->id) }}"
                                        onsubmit="return confirm('¿Eliminar {{ addslashes($producto->nombre) }}?')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="text-red-500 hover:text-red-400 text-xs uppercase tracking-wider">Eliminar</button>
                                    </form>
                                </div>
                            </td>
                            @endif
                        ...
                        @empty
                        <tr><td colspan="7" class="px-6 py-10 text-center text-gray-600">No hay productos registrados aún.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm">
                    ...
                    <tbody>
                        ...
                        @empty
                        <tr><td colspan="4" class="px-6 py-10 text-center text-gray-600">No hay compras registradas aún.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
    ...
    <div id="modal-comprar" class="hidden fixed inset-0 bg-black/70 flex items-center justify-center z-50">
        <div class="bg-gray-900 border border-gray-700 p-8 w-full max-w-md">
            ...
            <form method="POST" action="{{ route('economato.comprar') }}">
                ...
                <div class="mb-6">
                    <label class="block text-gray-400 text-xs uppercase tracking-wider mb-2">Cantidad <span id="modal-comprar-stock" class="text-gray-600 normal-case text-xs"></span></label>
                ...
                <div class="flex gap-3 justify-end">
                    <button type="button" onclick="document.getElementById('modal-comprar').classList.add('hidden')"
                    ...
                    <button type="submit" class="px-5 py-2.5 text-sm bg-yellow-500 text-gray-950 font-medium hover:bg-yellow-400 transition uppercase tracking-wider">Confirmar</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div id="modal-editar-producto" class="hidden fixed inset-0 bg-black/70 flex items-center justify-center z-50">
        <div class="bg-gray-900 border border-gray-700 p-8 w-full max-w-md">
            ...
            <form method="POST" id="form-editar" action="">
                ...
                <div class="mb-4">
                    <label class="block text-gray-400 text-xs uppercase tracking-wider mb-2">Nombre</label>
                    <input type="text" name="nombre" id="editar-nombre" required class="w-full bg-gray-800 border border-gray-700 text-white px-4 py-2.5 focus:outline-none focus:border-yellow-500">
                </div>
                <div class="mb-4">
                    <label class="block text-gray-400 text-xs uppercase tracking-wider mb-2">Tipo</label>
                    <input type="text" name="tipo" id="editar-tipo" required class="w-full bg-gray-800 border border-gray-700 text-white px-4 py-2.5 focus:outline-none focus:border-yellow-500">
                </div>
                <div class="mb-6">
                    <label class="block text-gray-400 text-xs uppercase tracking-wider mb-2">Stock</label>
                    <input type="number" name="stock" id="editar-stock" min="0" required class="w-full bg-gray-800 border border-gray-700 text-white px-4 py-2.5 focus:outline-none focus:border-yellow-500">
                </div>
                <div class="flex gap-3 justify-end">
                    <button type="button" onclick="document.getElementById('modal-editar-producto').classList.add('hidden')"
                        class="px-5 py-2.5 text-sm text-gray-400 border border-gray-700 hover:border-gray-500 transition uppercase tracking-wider">Cancelar</button>
                    <button type="submit" class="px-5 py-2.5 text-sm bg-yellow-500 text-gray-950 font-medium hover:bg-yellow-400 transition uppercase tracking-wider">Guardar cambios</button>
                </div>
            </form>
        </div>
    </div>
